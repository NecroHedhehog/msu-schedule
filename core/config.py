...
# Сколько часов данные считаются свежими. Порог у каждого прохода свой:
# один общий давал ложные тревоги, потому что ходят они с разной частотой.
# Значение берётся с запасом на худший разрыв в расписании таймеров.
FRESHNESS_MAX_HOURS = {
    'socio':           18,   # три прогона в день, ночной разрыв 19:10→07:10
    'socio-subgroups': 36,   # раз в сутки
    'socio-teachers':  96,   # Пн/Ср/Пт, худший разрыв пятница→понедельник
}
# Проходы, за свежестью которых следить не надо: запускаются руками
# раз в семестр, и «устарели» для них — нормальное состояние, а не тревога.
FRESHNESS_SKIP = ('socio-students',)

# Слать ли уведомление об успешном прогоне. По умолчанию нет: успех
# случается несколько раз в день, и среди этих «всё хорошо» теряется
# единственное сообщение, ради которого алерты и заводились.
ALERT_ON_SUCCESS = os.getenv('ALERT_ON_SUCCESS', '').lower() in ('1', 'true', 'yes')

# Через сколько часов повторять алерт с тем же самым текстом. Пустые группы
# держатся неделями, и напоминание о них трижды в день — шум, а не сигнал.
ALERT_REPEAT_HOURS = float(os.getenv('ALERT_REPEAT_HOURS', '24'))
